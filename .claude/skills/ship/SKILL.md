...
4. **`lanes N`.** `tools/release.py lanes <V> [--evidence <sweep.json>] [--priority <file>] [--max-per-lane <n>]` groups the lane-bound bugs into disjoint file groups by cited path and orders them; a bug with no cited path runs exclusively, with no other lane in flight (ADR-0085 D3's serialize-on-unknown default). A group of more than `--max-per-lane` bugs (default 10) comes back as several sub-lanes that share one `group` id.
